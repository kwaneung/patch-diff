import { Suspense } from 'react';
import { PatchCard } from '@/components/patch-card';
import { ModeSelector } from '@/components/mode-selector';
import { MainSearchInput } from '@/components/main-search-input';
import { getPatches } from '@/lib/data/patches';
import type { GameMode } from '@/lib/data/patches';

interface HomeProps {
  searchParams: Promise<{ mode?: string }>;
}

async function HomeContent({ searchParams }: HomeProps) {
  const params = await searchParams;
  const gameMode: GameMode =
    params.mode === 'tft' ? 'tft' : params.mode === 'aram-mayhem' ? 'aram-mayhem' : 'summoners-rift';
  const patches = await getPatches(gameMode);

  return (
    <>
      <div className="flex flex-col items-center mb-[2.5rem] text-center">
        <h1 className="text-[1.875rem] @[40rem]:text-[2.25rem] @[64rem]:text-[3rem] font-extrabold tracking-tight mb-[1rem] bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">
          PatchDiff.gg
        </h1>
        <p className="text-base @[40rem]:text-xl text-muted-foreground max-w-[37.5rem]">
          League of Legends 패치 노트를 <br className="hidden @[40rem]:inline"/>
          <span className="font-semibold text-foreground">팩트 중심</span>으로 빠르고 명확하게 확인하세요.
        </p>
        <Suspense fallback={<div className="h-10 mt-[1rem]" />}>
          <ModeSelector />
        </Suspense>
        <div className="mt-[1rem] w-full">
          <Suspense fallback={<div className="h-10" />}>
            <MainSearchInput />
          </Suspense>
        </div>
      </div>

      <div className="grid grid-cols-1 @[40rem]:grid-cols-2 @[64rem]:grid-cols-3 gap-[1.5rem]">
        {patches.map((patch) => (
          <PatchCard key={patch.id} patch={patch} gameMode={gameMode} />
        ))}
      </div>

      {patches.length === 0 && (
        <div className="text-center py-[5rem] text-muted-foreground">
          <p>아직 등록된 패치 데이터가 없습니다.</p>
        </div>
      )}
    </>
  );
}

export default function Home(props: HomeProps) {
  return (
    <div className="@container w-full max-w-5xl mx-auto py-[2.5rem] px-[1rem] @[40rem]:px-[1.5rem] @[64rem]:px-[2rem]">
      <Suspense fallback={<div className="animate-pulse py-20 text-center text-muted-foreground">로딩 중...</div>}>
        <HomeContent {...props} />
      </Suspense>
    </div>
  );
}